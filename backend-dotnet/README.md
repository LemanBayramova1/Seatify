...
table status. Lives alongside the existing Node/Prisma backend in `../backend` — nothing there
was touched.
...
src/
  ...
  Seatify.Api             controllers, SignalR hub, JWT/Swagger/CORS setup, Program.cs
...
```
cd backend-dotnet
dotnet run --project src/Seatify.Api
```
...
`src/Seatify.Api/appsettings.json`:
...
dotnet ef database update --project src/Seatify.Infrastructure --startup-project src/Seatify.Api
```
...
## Config (`src/Seatify.Api/appsettings.json`)
