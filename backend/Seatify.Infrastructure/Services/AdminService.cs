...
public class AdminService : IAdminService
{
    ...
    public AdminService(AppDbContext db, IDistributedLockService redisLock, ITableStateNotifier notifier, INotificationService notificationService)
    {
        _db = db;
    ...
    public async Task ToggleVenueActiveAsync(Guid venueId, bool isActive)
    {
        ...
        var wasActive = venue.IsActive;
        venue.IsActive = isActive;
        await _db.SaveChangesAsync();

        // Only the inactive-to-active transition counts as the venue "opening" — creating an
        // already-active venue is covered separately by VenueService.CreateAsync.
